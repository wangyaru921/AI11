...
# 需要从缓存中获取数据进行界面的渲染
list = dd.query_message_by_user_id_translator(user_id=user_id)
if list:
    ...
    with st.chat_message("assistant"):
        st.write("我是你的智能AI助手，可以回答你有关计算机的问题，请问你有什么问题？")


if len(st.session_state.messages) <=0 :
    with st.chat_message("assistant"):
        st.write("你好，我是你的专属计算机助手，可以回答你的任何的计算机问题")

input = st.chat_input("请输入你要咨询的计算机问题")
if input:
    with st.chat_message("user"):
        st.write(input)
        dd.add_chat_message_translator(user_id, input, "user")
        res = mm.invoke(input)
    st.session_state.messages.append({"role":"user","context":input})
    result = mm.chain_invoke({"context":input})
    with st.chat_message("assistant"):
        st.write(result)
        dd.add_chat_message_translator(user_id, res, "assistant")
